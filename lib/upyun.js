...
Upyun.prototype.info = function(filepath, callback) {
    ...
    spidex.method("head", this.baseUri + "/" + filepath, function(html, status, respHeader) {
        if(404 === status) {
...
Upyun.prototype.mkdir = function(directory, autoParentFolder, callback) {
    ...
    spidex.post(this.baseUri + "/" + directory, function(html, status, respHeader) {
        if(200 !== status) {
            return callback(new Error(html.replace("</h1>", "</h1> ").stripTags()));
        }

        callback(undefined, directory);
    }, {}, header, "utf8").on("error", callback);
};
...
Upyun.prototype.list = function(directory, callback) {
    ...
    spidex.get(this.baseUri + "/" + directory, function(html, status, respHeader) {
        if(404 === status) {
            return callback(new Error("File not found."));
        }
...
Upyun.prototype.fetchUsage = function(callback) {
    ...
    spidex.get(this.baseUri + "/?usage", function(html, status, respHeader) {
        if(404 === status) {
            return callback(new Error("File not found."));
        }
...
Upyun.prototype.rm = function(filepath, callback) {
    ...
    spidex.delete(this.baseUri + "/" + filepath, function(html, status, respHeader) {
        if(status !== 200) {
            return callback(new Error(html.replace("</h1>", "</h1> ").stripTags()));
        }
...
Upyun.prototype.download = function(filepath, callback) {
    ...
    spidex.get(this.baseUri + "/" + filepath, function(html, status, respHeader) {
        // 404 file not found
        if(404 === status) {
            return callback(new Error("File not found."));
        }
    ...
    }, this.baseHeader, "binary").on("error", callback);
};
